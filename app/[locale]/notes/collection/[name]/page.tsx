...
export async function generateStaticParams() {
  ...
  const collections = Array.from(
    new Set(notes.map((n) => n.metadata.collection).filter(Boolean))
  );
...
export default async function CollectionPage({
  ...
  const { notes: initialNotes, hasMore } = await getPaginatedNotes(
    filters,
    1,
    10
  );
  ...
  const collectionNotes = allNotes.filter(
    (n) => n.metadata.collection === collectionName
  );
  const allTags = Array.from(
    new Set(collectionNotes.flatMap((n) => n.metadata.tags || []))
  ) as string[];
  ...
  if (searchParams.note) {
    const [noteResult, adjacent] = await Promise.all([
      getPostBySlug<NoteMetadata>(searchParams.note, "notes", locale),
    ...
    <div className="min-h-screen">
      <BreadcrumbStructuredData
        items={[
          ...
          {
            name: collectionName,
            url: `${defaultSEOConfig.siteUrl}/notes/collection/${name}`,
          },
        ]}
      />
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
            <div className="space-y-4">
              ...
              <p className="text-xl text-muted-foreground leading-relaxed max-w-2xl">
                A curated stream of thoughts and discoveries from the{" "}
          ...
          {initialNotes.length > 0 ? (
            ...
            <div className="text-center py-24 border rounded-2xl bg-muted/30 border-dashed">
              <p className="text-muted-foreground italic">
                No notes found matching your filters in this collection.
              </p>
            </div>
          )}
        </section>
