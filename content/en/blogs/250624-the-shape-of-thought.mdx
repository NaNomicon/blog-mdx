...
*   **The "Pit of Success":** A well-designed API creates a "pit of success" - it's easier to use it correctly than incorrectly. The names of functions, the structure of the data, and the error messages all guide the developer toward a solid implementation. A poorly designed API, on the other hand, is a "pit of failure," where intuitive use leads to bugs. The medium itself is either a guide or a trap.
